...
import { ArrowLeft, Check, IndianRupee, PlusCircle, MinusCircle } from 'lucide-react-native';
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingTop: 56, paddingBottom: 16, backgroundColor: theme.card, borderBottomWidth: 1, borderBottomColor: theme.border }}>
        ...
        <Text style={{ fontSize: 22, fontWeight: '700', color: theme.ink, letterSpacing: -0.5 , fontFamily: 'FjallaOne_400Regular'}}>Update Debt</Text>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} automaticallyAdjustKeyboardInsets={true} contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        ...
        <View style={{ backgroundColor: theme.card, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: theme.border, marginBottom: 24, alignItems: 'center' }}>
          <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: theme.warning + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
            <Text style={{ fontSize: 24, fontWeight: '900', color: theme.warning , fontFamily: 'FjallaOne_400Regular'}}>{person.charAt(0).toUpperCase()}</Text>
          </View>
          <Text style={{ fontSize: 16, fontWeight: '700', color: theme.muted, marginBottom: 4 , fontFamily: 'FjallaOne_400Regular'}}>Current Balance with {person}</Text>
          <Text style={{ fontSize: 32, fontWeight: '900', color: theme.ink, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}}>
        ...
        <View style={{ backgroundColor: theme.surface, borderRadius: 18, padding: 5, flexDirection: 'row', borderWidth: 1, borderColor: theme.border, marginBottom: 24 }}>
          <TouchableOpacity onPress={() => setActionType('pay')} activeOpacity={0.8}
            ...
            <Text style={{ fontSize: 15, fontWeight: '900', color: actionType === 'pay' ? '#fff' : theme.muted , fontFamily: 'FjallaOne_400Regular'}}>
              Log Payment
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActionType('add')} activeOpacity={0.8}
            ...
            <Text style={{ fontSize: 15, fontWeight: '900', color: actionType === 'add' ? '#fff' : theme.muted , fontFamily: 'FjallaOne_400Regular'}}>
              Add Amount
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ marginBottom: 18 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <Text style={{ fontSize: 11, fontWeight: '800', color: theme.muted, letterSpacing: 1.5, textTransform: 'uppercase' , fontFamily: 'FjallaOne_400Regular'}}>
              {actionType === 'pay' ? 'Amount Paid (₹)' : 'Extra Amount (₹)'}
            </Text>
          </View>
          <TextInput 
            style={{ backgroundColor: theme.card, borderRadius: 18, padding: 16, color: activeColor, fontSize: 28, fontWeight: '900', borderWidth: 1, borderColor: theme.border, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}}
        ...
        <Text style={{ fontSize: 13, color: theme.muted, textAlign: 'center', marginTop: 12 , fontFamily: 'FjallaOne_400Regular'}}>
          {actionType === 'pay' 
      ...
      <View style={{ paddingHorizontal: 20, paddingVertical: 16, paddingBottom: 36, borderTopWidth: 1, borderTopColor: theme.border, backgroundColor: theme.card }}>
        <TouchableOpacity onPress={onSubmit} activeOpacity={0.85}
          ...
          >
            ...
            <Text style={{ color: '#fff', fontWeight: '700', fontSize: 16 , fontFamily: 'FjallaOne_400Regular'}}>Update Record</Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
