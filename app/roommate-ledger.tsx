...
import { ArrowLeft, Plus, X, Check, Edit2, Trash2, ArrowUpRight, ArrowDownLeft, CheckCircle, Circle, MessageCircle , Receipt} from 'lucide-react-native';
    ...
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 56, paddingBottom: 16, backgroundColor: theme.card, borderBottomWidth: 1, borderBottomColor: theme.border }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 20, fontWeight: '700', color: theme.ink, letterSpacing: -0.5 , fontFamily: 'FjallaOne_400Regular'}}>{roommateName}</Text>
            <Text style={{ fontSize: 12, fontWeight: '600', color: theme.muted, marginTop: 2 , fontFamily: 'FjallaOne_400Regular'}}>Roommate Ledger</Text>
          </View>
        </View>
        <TouchableOpacity onPress={shareViaWhatsApp} activeOpacity={0.75}
          ...
          <Text style={{ color: '#25D366', fontWeight: '800', fontSize: 13 , fontFamily: 'FjallaOne_400Regular'}}>Share</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{
          ...
          {netBalance === 0 ? (
            <View>
              ...
              <Text style={{ fontSize: 14, color: theme.muted, fontWeight: '600', marginTop: 4 , fontFamily: 'FjallaOne_400Regular'}}>No pending dues between you two</Text>
            </View>
          ) : netBalance > 0 ? (
            <View>
              <Text style={{ fontSize: 24, fontWeight: '900', color: theme.success, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}}>
                ₹{absBalance.toLocaleString('en-IN')}
              </Text>
              <Text style={{ fontSize: 14, color: theme.success, fontWeight: '700', marginTop: 4 , fontFamily: 'FjallaOne_400Regular'}}>
                {roommateName} owes you
              </Text>
            </View>
          ) : (
            <View>
              <Text style={{ fontSize: 24, fontWeight: '900', color: theme.danger, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}}>
                ₹{absBalance.toLocaleString('en-IN')}
              </Text>
              <Text style={{ fontSize: 14, color: theme.danger, fontWeight: '700', marginTop: 4 , fontFamily: 'FjallaOne_400Regular'}}>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 24 }}>
          <View style={{ flex: 1, backgroundColor: theme.card, borderRadius: 16, padding: 14, borderWidth: 1, borderColor: theme.border }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              ...
              <Text style={{ fontSize: 10, fontWeight: '800', color: theme.muted, textTransform: 'uppercase' , fontFamily: 'FjallaOne_400Regular'}}>You Paid</Text>
            </View>
            <Text style={{ fontSize: 16, fontWeight: '900', color: theme.success, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}} adjustsFontSizeToFit numberOfLines={1}>
          ...
          <View style={{ flex: 1, backgroundColor: theme.card, borderRadius: 16, padding: 14, borderWidth: 1, borderColor: theme.border }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              ...
              <Text style={{ fontSize: 10, fontWeight: '800', color: theme.muted, textTransform: 'uppercase' , fontFamily: 'FjallaOne_400Regular'}}>{roommateName} Paid</Text>
            </View>
            <Text style={{ fontSize: 16, fontWeight: '900', color: theme.danger, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}} adjustsFontSizeToFit numberOfLines={1}>
        ...
        {entries.length === 0 ? (
          <View style={{ backgroundColor: theme.card, borderRadius: 20, padding: 32, alignItems: 'center', borderWidth: 1, borderColor: theme.border, borderStyle: 'dashed' }}>
            <Receipt size={32} color={theme.primary} style={{ marginBottom: 12 }} />
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1, marginRight: 10 }}>
                    <Text style={{ fontSize: 15, fontWeight: '700', color: theme.ink, marginBottom: 3, textDecorationLine: isPaid ? 'line-through' : 'none' , fontFamily: 'FjallaOne_400Regular'}} numberOfLines={1}>
                      {entry.description}
                    </Text>
                  ...
                  {/* Amount */}
                  <Text style={{ fontSize: 16, fontWeight: '900', color: entryColor, fontVariant: ['tabular-nums'] , fontFamily: 'FjallaOne_400Regular'}}>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          <View style={{ backgroundColor: theme.background, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, minHeight: 420 }}>
            ...
            {/* Who Paid Toggle */}
            <Text style={{ color: theme.muted, fontWeight: '700', marginBottom: 8, fontSize: 13 , fontFamily: 'FjallaOne_400Regular'}}>Who paid?</Text>
              ...
              {(['me', 'roommate'] as const).map(opt => {
                ...
                return (
                  <TouchableOpacity key={opt} onPress={() => setFormPaidBy(opt)} activeOpacity={0.8}
                    ...
                    <Text style={{ fontSize: 14, fontWeight: '900', color: active ? '#fff' : theme.muted , fontFamily: 'FjallaOne_400Regular'}}>
