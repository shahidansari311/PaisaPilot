...
export default function AddBorrowLend() {
  ...
  const activeAccent = selectedType === 'borrowed' ? theme.danger : theme.success;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <Field icon={<User size={16} color={theme.muted} />} label="Person Name" error={errors.person?.message}>
          <Controller control={control} name="person" render={({ field: { onChange, value } }) => (
            <TextInput style={inputStyle} placeholder="e.g. Arjun Sharma" placeholderTextColor={theme.muted} value={value} onChangeText={onChange} />
          )} />
        </Field>
        ...
        <Field icon={<Calendar size={16} color={theme.muted} />} label="Due Date (YYYY-MM-DD)" error={errors.dueDate?.message}>
          <Controller control={control} name="dueDate" render={({ field: { onChange, value } }) => (
            <TextInput style={inputStyle} placeholder="2026-12-31" placeholderTextColor={theme.muted} value={value} onChangeText={onChange} keyboardType="numeric" />
          )} />
          {/* Quick shortcuts */}
