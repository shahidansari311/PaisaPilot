...
export default function Settings() {
  ...
  const SectionLabel = ({ emoji, title }: { emoji: string; title: string }) => (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 16, marginTop: 8 }}>
      ...
      <Text style={{ fontSize: 14, fontWeight: '800', color: muted, textTransform: 'uppercase', letterSpacing: 1.5 }}>{title}</Text>
    </View>
  );
  ...
  const SettingRow = ({ icon, label, onPress, right, isDanger = false, noBorder = false }: { icon: React.ReactNode; label: string; onPress?: () => void; right?: React.ReactNode; isDanger?: boolean; noBorder?: boolean }) => (
    <TouchableOpacity onPress={onPress} activeOpacity={onPress ? 0.7 : 1}
      ...
      <Text style={{ flex: 1, fontSize: 16, fontWeight: '700', color: isDanger ? danger : ink }}>{label}</Text>
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: bg }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <View style={{ paddingHorizontal: 24, paddingTop: 64, paddingBottom: 24 }}>
        <Text style={{ fontSize: 32, fontWeight: '900', color: ink, letterSpacing: -1 }}>Settings ⚙️</Text>
        <Text style={{ fontSize: 14, fontWeight: '600', color: muted, marginTop: 4 }}>Customize your experience</Text>
      </View>
      ...
      <View style={{ paddingHorizontal: 24 }}>
        ...
        <Text style={{ fontSize: 13, fontWeight: '600', color: muted, textAlign: 'center', marginBottom: 8 }}>
          PaisaPilot v1.0.0
        </Text>
        <Text style={{ fontSize: 12, color: muted, textAlign: 'center' }}>Built for students 💜</Text>
      </View>
    </ScrollView>
  );
}
